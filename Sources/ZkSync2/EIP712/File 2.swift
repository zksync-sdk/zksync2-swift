...
extension AbstractEnvelope {
    ...
    var publicKey: Data? {
        ...
        let vData = BigUInt(sigData.v).serialize().setLengthLeft(1)
        let rData = sigData.r
        let sData = sigData.s
...
public struct EnvelopeFactory {
    ...
    static func createEnvelope(rawValue: Data) -> Any? {
    ...
    /// - Returns: a transaction envelope according to the type dictated by the input data
    static func createEnvelope(from decoder: Decoder) throws -> Any? {
    ...
    static func createEnvelope(type: TransactionType? = nil, to: EthereumAddress, nonce: BigUInt,
                               ...
                               accessList: [AccessListEntry]?, v: BigUInt, r: BigUInt, s: BigUInt) -> Any {
        ...
        case .eip712: return EIP712Envelope(to: to, chainID: chainID, value: value, data: data, maxPriorityFeePerGas: maxPriorityFeePerGas ?? 0, maxFeePerGas: maxFeePerGas ?? 0, gasLimit: gasLimit, accessList: accessList ?? [], v: v, r: r, s: s)
