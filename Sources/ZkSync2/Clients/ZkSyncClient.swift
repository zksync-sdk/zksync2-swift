...
public protocol ZkSyncClient {
    ...
    func getTransferTx(_ to: String, amount: BigUInt, from:String, token: String?, options: TransactionOption?, paymasterParams: PaymasterParams?) async -> CodableTransaction
    // EstimateGasTransfer estimates the amount of gas required for a transfer
    // transaction.
    func estimateGasTransfer(_ to: String, amount: BigUInt, from:String, token: String?, options: TransactionOption?, paymasterParams: PaymasterParams?) async -> BigUInt
}
